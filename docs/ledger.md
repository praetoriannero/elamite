...
| Reference formation, receiver adaptation, slice coercion, closures, and iterator state do not allocate | **Structural borrow provenance** (done), **Promotion and tracing-GC removal** (done) | — | `owned_reference_formation_accepts_composite_storage_and_rejects_mutable_shared_reborrows`, `owned_reference_temporaries_and_variadic_packs_use_stack_storage` |
...
| `[T]` is a shared slice, `[var T]` an exclusive slice, and `[T; N]` an owning fixed array | **Migration seam and accepted surface** (surface/lowering done), **Owned core values and collections** (done) | — | `source_type_lowering_distinguishes_shared_mutable_and_owning_sequences`; owned-core shared/exclusive runtime case |
| `String`, `Vec`, `Map`, and `Set` have unique ownership, constant-size moves, explicit content clones, and exact destruction | **Owned core values and collections** (done), **Deterministic destruction** (done) | allocator | `owned_core_values_clone_move_mutate_and_drop_independently`; ASan and cost workloads |
| Collection indexing cannot move a dynamic non-`Copy` element; removal/take APIs transfer ownership and borrows block relocation | **Move and initialization checking** (indexed-move rejection done), **Structural borrow provenance** (relocation conflicts done), **Owned core values and collections** (done) | bounds traps | `move_checking_tracks_loops_and_rejects_indexed_collection_moves`; owned-core `remove`/`pop` and live-slice relocation cases |
| Owned iteration consumes and moves elements; borrowed iteration yields references whose provenance prevents invalidation | **Owned core values and collections** (done), **Structural borrow provenance** (done) | — | owned-core early-break and borrowed-slice run-pass; borrow conflict suite |
...
| `Copy`, `Send`, and `Sync` are structural capabilities; `Clone` and `Drop` are coherent traits with specified compiler hooks | **Ownership facts and explicit use IR** (done), **Deterministic destruction** (done), **Race-safe concurrency** (done) | — | canonical recursive/generic/error assertions, direct-hook-call rejection, thread-boundary capability cases |
...
| `fn[captures](args):` creates an inline nominal first-class closure object; `fn[]` is capture-free; captures are exhaustive and evaluate left-to-right | **Inline first-class closures** (done) | — | `owned_surface_parses_closures_slices_arrays_and_borrows_without_legacy_leakage`, `owned_closures_are_inline_shared_callable_values` |
| Plain closure captures move or `Copy`; `&`/`&var` capture provenance; construction and ordinary call allocate nothing | **Inline first-class closures** (done), **Structural borrow provenance** (done) | — | `borrow_provenance_flows_through_closure_environments_and_calls`, `owned_closure_construction_is_allocation_free_and_capture_moves_are_rejected` |
| One `Callable[Args, Return]` shared-call contract covers repeated calls; mutable state is explicit through captured `&var` or synchronized types | **Inline first-class closures** (done) | — | `owned_closures_are_inline_shared_callable_values`, capture-move compile-fail |
| Capture-free closures may explicitly become exact function references or raw callback pointers; capturing closures use a raw context for C callbacks | **Inline first-class closures** (done), **Owned-model C interoperability** (done) | — | `owned_closures_are_inline_shared_callable_values`; `owned_foreign_signatures_and_uninitialized_storage_are_checked`; native callback harness |
...
| `Box` uniquely owns address-stable storage; `Shared`/`Weak` explicitly reference-count sharing; strong cycles require `Weak` or a store | **Explicit shared and graph ownership** (done) | allocator, synchronized refcounts | `shared_weak_and_generational_store_run_with_exact_cleanup`, cost workload |
| `Store[T]` owns homogeneous graph nodes; `Handle[T]` is copyable store/slot/generation identity; wrong or stale lookup traps | **Explicit shared and graph ownership** (done) | store runtime | `stale_and_wrong_store_handles_trap_deterministically`, `handle_layout_is_pointer_width_neutral_c99` |
| No tracing collector, promotion, collector rooting, or thread registration remains | **Promotion and tracing-GC removal** (done) | allocator only | `compiler_sources_contain_no_collector_or_root_hook`, generated-C assertions, ASan/LSan |
| Allocation sites are explicit in owning APIs; OOM is fatal and non-unwinding | **Owned core values and collections** (done), **Promotion and tracing-GC removal** (done) | allocator, OOM terminator | cost model, owned-core C99 run, collector-free process-allocation harness |
...
| Closure syntax is revisioned: the 0.10 compatibility form remains historical, while 0.11 uses `fn[captures](parameters) -> Return:`, including `fn[]`; parameters are typed, return annotation optional; no implicit, generic, unsafe, or variadic closure forms | Closures: 0.10 baseline plus **Inline first-class closures** (done) | — | parser snapshot, owned surface and generated-C run-pass/compile-fail |
...
| The 0.10 compatibility path shallow-copies captures into one managed identity-sharing environment. The 0.11 path stores capture representations in one nominal inline aggregate; plain captures move or `Copy`, borrowed captures carry provenance, construction allocates nothing, and structural clone/drop follows field capabilities | 0.10 closures baseline; **Inline first-class closures** (done) | compatibility Boehm GC only | compatibility regression; `owned_closures_are_inline_shared_callable_values`, allocation/capture-move conformance |
...
| A 0.11 capture-free `fn[]` closure explicitly converts only to its exact safe function reference or matching raw function pointer; capturing closures never convert to either or masquerade as C callbacks; callable equality/hash, `CallableMut`/`CallableOnce`, private evolving/initialized/default captures remain unsupported | **Inline first-class closures** (done), **Owned-model C interoperability** (done) | — | exact conversion run-pass; owned native callback harness; capturing/mismatched conversion compile-fail |
...
| Safe references are not ABI-safe and cross the boundary only as an explicit raw-pointer conversion; a non-retaining foreign call keeps the owner or borrow live for the call; retained-after-return storage uses address-stable `Box` or `Shared` ownership held by the wrapper | **Owned-model C interoperability** (done) | explicit owner | owned native C harness |
| `Box.pointer`/`pointer_var` expose nonowning raw addresses, `into_raw` visibly transfers cleanup responsibility, and unsafe `from_raw` restores it only for an allocation with the exact compatible allocator contract; foreign allocations instead live in move-only wrappers whose `Drop` invokes their exact native deleter | **Owned-model C interoperability** (done) | allocator and foreign library | ownership round-trip plus exact resource-release counter under ASan/UBSan |
| `std.ffi.MaybeUninit[T]` is move-only, logically uninitialized ABI-aligned storage; `pointer` requires mutable access and unsafe `assume_init` consumes the storage after wrapper code has verified the foreign success condition | **Owned-model C interoperability** (done) | C ABI | compile-fail unsafe/ABI cases and initialized output harness |
| `ForeignRoot`/`ForeignRootMut` and their process-lifetime compatibility state remain callable only on 0.10; the owned path has no promotion or root-registration escape hatch | **Owned-model C interoperability** (done), **Promotion and tracing-GC removal** (done) | compatibility process registry only | owned compile-fail plus retained 0.10 backend cases |
...
| Rule | Pass | Runtime | Tests |
| --- | --- | --- | --- |
| `@exportc("c_name")` gives an ABI-safe module-level function definition an exact unmangled link symbol without creating a separate function kind; exact named functions, references, and capture-free closures can explicitly become raw callback pointers | M17 plus **Owned-model C interoperability** (done) | — | exported pthread harness and direct capture-free callback run-pass |
| Stateful callbacks retain an address-stable `Box`/`Shared` context owner in a move-only registration wrapper; the callback recovers a reference only within `unsafe:`, and wrapper destruction unregisters before releasing context | **Owned-model C interoperability** (done) | explicit owner and foreign API | owned resource/context pattern; capturing conversion compile-fail |
| Foreign-created threads may enter through an exported function or registered callback address without collector attachment; unsafe registration remains responsible for `Send`/`Sync`, synchronization, and lifetime obligations | **Owned-model C interoperability** (done), **Race-safe concurrency** (done) | pthread runtime | `a_foreign_created_thread_may_enter_through_an_exported_boundary` |
...
| `std.ffi.ForeignRoot[T]`, `ForeignRootMut[T]` | compatibility retained-pointer state | preserves the 0.10 process-lifetime registration API while the owned path requires explicit address-stable ownership (§10.2) | M17; **Owned-model C interoperability** (restricted) |
| `std.ffi.MaybeUninit[T]` | C output storage | ABI-aligned logical uninitialization, mutable raw output pointer formation, and unsafe consuming initialization proof (§10.1–10.2) | **Owned-model C interoperability** (done) |
