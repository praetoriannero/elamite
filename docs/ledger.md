...
| --- | --- | --- | --- |
| 0.11 is the accepted target while one complete 0.10 path remains as temporary migration scaffolding | **Migration seam and accepted surface** (done), **Ownership facts and explicit use IR** (done) | — | `semantic_revision` package/phase integration, dependency-skew diagnostic, single post-ownership boundary |
| Non-`Copy` consuming contexts move; `Copy` values remain available; no hidden clone | **Ownership facts and explicit use IR** (operation selection done), **Move and initialization checking** | — | `ownership_facts_places_and_operations_cross_both_ir_levels`; later compile-pass/fail |
