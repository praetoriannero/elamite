...
| Allocation sites are explicit in owning APIs; OOM is fatal and non-unwinding | **Owned core values and collections** (owning APIs done), **Promotion and tracing-GC removal** | allocator, OOM terminator | cost model, owned-core C99 run; final collector-removal trap-process harness |
