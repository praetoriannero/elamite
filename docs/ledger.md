...
| `String`, `str` | text types | literal contextual materialization, `StableHash` asymmetry, ABI-unsafety; the planned **Source-hosted standard library** milestone narrows implementation privilege to a private UTF-8 traversal, slicing, and construction kernel and moves text algorithms into Elamite | M14; **Source-hosted standard library** (planned) |
