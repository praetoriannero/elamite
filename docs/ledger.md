...
| Capture-free closures may explicitly become exact function references; capturing closures use a raw context for C callbacks | **Inline first-class closures** (conversion done), **Owned-model C interoperability** | — | `owned_closures_are_inline_shared_callable_values`; capturing-conversion compile-fail, later C harness |
...
| A 0.11 capture-free `fn[]` closure explicitly converts only to its exact safe function reference; capturing closures never convert to function references, raw function pointers, or C callbacks; callable equality/hash, `CallableMut`/`CallableOnce`, private evolving/initialized/default captures remain unsupported | **Inline first-class closures** (done), callback ownership remains **Owned-model C interoperability** | — | exact conversion run-pass; capturing/mismatched conversion compile-fail |
