...
| --- | --- | --- | --- |
| 0.11 is the accepted target while one complete 0.10 path remains as temporary migration scaffolding | **Migration seam and accepted surface** (done), **Ownership facts and explicit use IR** (done), **Move and initialization checking** (done) | — | `semantic_revision` package/phase integration, dependency-skew diagnostic, single post-move boundary |
...
| Custom-destruction types are non-`Copy` and cannot be partially moved | **Move and initialization checking** (partial-move rejection done), **Deterministic destruction** | — | `move_checking_rejects_partial_moves_from_custom_drop_types`; later drop trace |
...
| Shared references are `Copy`; exclusive references are move-only and support shorter reborrows | **Ownership facts and explicit use IR** (facts/operations done), **Structural borrow provenance** | — | `ownership_facts_places_and_operations_cross_both_ir_levels`; later compile-pass/fail |
...
| `Copy`, `Send`, and `Sync` are structural capabilities; `Clone` and `Drop` are coherent traits with specified compiler hooks | **Ownership facts and explicit use IR** (canonical query done), **Deterministic destruction**, **Race-safe concurrency** | — | canonical recursive/generic/error capability assertions; later compile-pass/fail |
