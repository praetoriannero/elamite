...
| `copies.elx` | 3.1 | shallow inline slots through five nesting levels, descriptor and explicit-alias identity through aggregate copies, argument/return aliasing, `&var` stored in a `let` aggregate |
...
| `collections.elx` | 4.1, 7.1 | insertion through `len` inclusive, removal below `len`, displaced-value reporting, duplicate literal keys and elements, struct and tuple `StableHash` keys, nested backing identity, vector growth divergence, shallow iteration state and visible element replacement |
...
| `closures.elx` | 5.1 | all five capture forms in one closure, capture evaluation order, shallow capture and shared environment identity under closure copies, return inference, `Callable` bounds and `&Callable` erasure, escaping environments, closure-local `defer` |
