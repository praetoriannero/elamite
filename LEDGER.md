...
| `std.ast` is a versioned compile-time-only façade of opaque immutable pre-resolution structural values, persistent lists, accessors/`with_` transforms, builders, pattern variants, origins, and `std.ast.error`; it exposes neither compiler AST/tables nor inferred types, layout, target, or runtime state | **Compile-time AST and interpreter** (`std.ast` façade done; language-level checking/execution remains later tasks) | compile-time intrinsics | unit/interface (`interface_versions_are_exact_and_inventory_is_stable`, `every_admitted_expression_pattern_type_and_statement_variant_is_constructible`, `every_structural_transform_preserves_the_original_value`, generated-origin diagnostics), property (`persistent_list_concatenation_preserves_arbitrary_inputs`, `every_noncurrent_interface_version_is_rejected`) |
| Typed `quote:` produces the expected AST role; `$name` and `$(expression)` interpolate with collection-position splicing; quote literals are definition-site hygienic, interpolated syntax retains context/provenance, and users cannot fabricate physical spans or call-site contexts | **Compile-time AST and interpreter** (syntax, explicit-role inference, scalar/splice classification, ordinary-parser structural validation, gating, formatting, and editor support done), **Interpreter-backed macros, attributes, and derives** (façade-value construction and hygiene context assignment) | compile-time interpreter | lexer/parser (`parses_quote_bodies_and_both_interpolation_forms_losslessly`, malformed and indentation recovery), expansion (`typed_quotes_validate_roles_only_inside_compile_time_declarations`, every role and specific-definition validation), property (`arbitrary_valid_quote_interpolations_parse_without_losing_sites`), formatter/editor; hygiene/provenance execution snapshots remain later |
...
| Physical compile-time environments are collected first; expansion uses package/module/provenance order, attributes-before-derives staging, outermost-first and left-to-right function macros, generated-item re-entry, delayed ordinary imports, and structural active-chain cycle detection | **Macro expansion foundations** (provenance and execution-independent scheduler done), **Interpreter-backed macros, attributes, and derives** (execution integration) | compile-time interpreter | scheduler unit tests (`ready_work_is_ordered_by_module_and_provenance_not_insertion`, `definition_stages_attributes_then_derives_then_invocations`, `generated_invocations_and_definitions_reenter_the_same_queue`, `equal_active_chain_input_recovers_as_a_cycle_but_changed_input_is_allowed`), fixed-point integration |
...
| User declarations and uses remain gated by `--unstable-macros`; collection macros, FFI attributes, and compact built-in derives stay ungated until the complete conformance/tooling audit permits stabilization | **Macro expansion foundations** (gate done), **Compile-time diagnostics, tooling, and stabilization** (ungate) | — | integration (`user_compile_time_forms_require_the_explicit_unstable_gate`, `compiler_macros_ffi_attributes_and_compact_derives_remain_ungated`, `command_line_requires_an_explicit_gate_for_user_defined_macros`), compatibility matrix |
