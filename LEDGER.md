...
- **Pass** — the `IMPL.md` milestone(s) expected to implement it, using the
  short tags below.
...
- **Tests** — which `IMPL.md` §2.4 test layer(s) should cover it: *parse*,
...
`IMPL.md` Milestone 0 calls for inventorying legacy grammar, compiler
...
introduced the Rust skeleton and `IMPL.md`. No legacy test fixtures, parser
...
| `Map` values have no `StableHash` requirement; no collection API exposes an interior safe reference; `Identity[&T]`/`Identity[&var T]` are compiler-known `StableHash` exceptions via managed target identity | M14 | Boehm GC | run-pass |
...
| Rule | Pass | Runtime | Tests |
| --- | --- | --- | --- |
| `Display` compiler-recognized prelude trait, formatting method writes to a mutable `Formatter`; user-implementable; primitives/`str`/`String`/references-to-displayable/std collections-of-displayable provide impls | M13, M14 | — | compile-fail (non-`Display` in `f"..."`) |
...
| `unsafe:` block permits raw dereference/raw→ref conversion/unsafe-or-foreign calls; doesn't disable type checking or prove validity; the author asserts the documented preconditions and §3.3 obligations hold | M16 | — | compile-fail (op outside `unsafe:`) |
| An unsafe-only op outside the required context is a compile-time error; §3.3's expression-local constant rule is the *only* mandatory value analysis for raw-pointer access; broader-analysis violations are warnings only; inability to prove valid foreign input is not itself an error; these diagnostics don't fire merely because a safely-formed local reference escapes (it's promoted instead) | M16 | — | compile-fail/compile-pass (escaping reference not flagged) |
...
Per `IMPL.md` Milestone 0, the initial supported-target assumptions:
...
Per `IMPL.md` Milestone 0, exact command spelling is a tooling decision; these
...
- **Parser generators** (`pest`, `lalrpop`, `chumsky`) — rejected for the core
  grammar. Every diagnostic-quality requirement in `IMPL.md` §2.3
...
- **Arena-of-references AST** (`bumpalo`, `typed-arena`) — would introduce
  ...
  references between phase data" (`IMPL.md` §2.1, `AGENTS.md`). A plain
